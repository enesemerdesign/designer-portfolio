    ...
    <section className="testimonials-stack relative pb-10">
      {testimonials.map((testimonial, index) => {
        ...
        const tagClasses = (type: 'featured' | 'default') => type === 'featured'
          ? 'bg-[var(--accent)]/20 text-[var(--fg)] border border-[var(--border-strong)]'
          : 'bg-[var(--bg-alt)] text-[var(--fg-muted)]';
            ...
            <div className="p-6 md:p-8">
              <div className="flex items-start justify-between mb-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-[var(--fg)] font-medium text-lg">{testimonial.name}</h3>
                    <p className="text-sm text-[var(--fg-muted)] mt-1">{testimonial.role}</p>
                  </div>
                </div>
              </div>

              <blockquote className="text-[var(--fg-mid)] leading-relaxed text-lg mb-6 font-sans">"{testimonial.quote}"</blockquote>

              <div className="flex flex-col md:flex-row items-start md:items-center justify-between border-t border-[var(--border)] pt-4 gap-4">
      ...
      <div className="pagination flex gap-2 justify-center absolute bottom-0 left-0 right-0">
        {testimonials.map((_, index) => (
          <button key={index} aria-label={`Yoruma git ${index + 1}`} onClick={() => navigate(index)} className={`pagination-dot ${activeIndex === index ? 'active' : ''}`} />
        ))}
      </div>
    </section>
  );
