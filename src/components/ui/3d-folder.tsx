...
const ProjectCard = forwardRef<HTMLDivElement, ProjectCardProps>(
  ({ image, title, delay, isVisible, index, totalCount, onClick, isSelected }, ref) => {
    ...
    return (
      ...
      >
        <div className={cn(
          "w-full h-full rounded-lg overflow-hidden shadow-xl bg-[var(--bg-card)] border border-white/5 relative",
          "transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)]",
          "group-hover/card:-translate-y-6 group-hover/card:shadow-2xl group-hover/card:shadow-[var(--accent)]/40 group-hover/card:ring-2 group-hover/card:ring-[var(--accent)] group-hover/card:scale-125"
        )}>
          <img
  ...
  return (
    ...
    >
      ...
      <button
        ...
        className={cn(
          "absolute top-6 right-6 z-50 w-12 h-12 flex items-center justify-center rounded-full bg-[var(--surface)]/30 backdrop-blur-xl border border-white/10 shadow-2xl text-[var(--fg)] hover:bg-[var(--surface)] transition-all duration-300",
        )}
        style={{
      ...
      <button
        ...
        className={cn(
          "absolute left-4 md:left-10 z-50 w-14 h-14 flex items-center justify-center rounded-full bg-[var(--surface)]/30 backdrop-blur-xl border border-white/10 text-[var(--fg)] hover:scale-110 active:scale-95 transition-all duration-300 disabled:opacity-0 disabled:pointer-events-none shadow-2xl",
        )}
        style={{
      ...
      <button
        ...
        className={cn(
          "absolute right-4 md:right-10 z-50 w-14 h-14 flex items-center justify-center rounded-full bg-[var(--surface)]/30 backdrop-blur-xl border border-white/10 text-[var(--fg)] hover:scale-110 active:scale-95 transition-all duration-300 disabled:opacity-0 disabled:pointer-events-none shadow-2xl",
        )}
        style={{
      ...
      >
        <div className={cn("relative overflow-hidden rounded-[inherit] bg-[var(--bg-card)] border border-white/10 shadow-[0_35px_60px_-15px_rgba(0,0,0,0.5)]")}>
          ...
          <div
            className={cn("px-8 py-7 bg-[var(--bg-card)] border-t border-white/5")}
            style={{
              opacity: animationPhase === "complete" && !isClosing ? 1 : 0,
          ...
          >
            <div className="flex items-center justify-between gap-6">
              <div className="flex-1 min-w-0">
                <h3 className="text-2xl font-bold text-[var(--fg)] tracking-tight truncate">{currentProject?.title}</h3>
                <div className="flex items-center gap-4 mt-2">
                  <div className="flex items-center gap-1.5 px-2.5 py-1 bg-[var(--surface)] rounded-full border border-white/5">
                    {projects.map((_, idx) => (
                      <button
                        key={idx}
                        onClick={() => handleDotClick(idx)}
                        className={cn("w-1.5 h-1.5 rounded-full transition-all duration-500", idx === internalIndex ? "bg-[var(--fg)] scale-150" : "bg-[var(--fg-muted)]/30 hover:bg-[var(--fg-muted)]/60")}
                      />
                    ))}
                  </div>
                  <p className="text-xs font-bold uppercase tracking-widest text-[var(--fg-muted)]/60">{internalIndex + 1} / {totalProjects}</p>
                </div>
              </div>
              <button className={cn("flex items-center gap-2 px-6 py-3 text-sm font-bold uppercase tracking-widest text-[var(--accent-fg)] bg-[var(--accent)] hover:brightness-110 rounded-xl shadow-lg shadow-[var(--accent)]/20 transition-all duration-300 hover:scale-105 active:scale-95")}>
...
export const AnimatedFolder: React.FC<AnimatedFolderProps> = ({ title, projects, className, gradient }) => {
  ...
  return (
    <>
      <div
        className={cn("relative flex flex-col items-center justify-center p-8 rounded-2xl cursor-pointer bg-[var(--bg-card)] border border-[var(--border)] transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] hover:shadow-2xl hover:shadow-[var(--accent)]/20 hover:border-[var(--accent)]/40 group", className)}
      ...
      >
        ...
        <div className="text-center">
          <h3 className="text-lg font-bold text-[var(--fg)] mt-4 transition-all duration-500" style={{ transform: isHovered ? "translateY(2px)" : "translateY(0)", letterSpacing: isHovered ? "-0.01em" : "0" }}>{title}</h3>
          <p className="text-sm font-medium text-[var(--fg-muted)] transition-all duration-500" style={{ opacity: isHovered ? 0.8 : 1 }}>{projects.length} proje</p>
        </div>
        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex items-center gap-1.5 text-xs font-semibold uppercase tracking-widest text-[var(--fg-extra-ghost)] transition-all duration-500" style={{ opacity: isHovered ? 0 : 1, transform: isHovered ? "translateY(10px)" : "translateY(0)" }}>
          <span>Hover</span>
        </div>
      </div>
