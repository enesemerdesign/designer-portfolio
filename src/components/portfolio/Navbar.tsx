        ...
        <div className="max-w-[1400px] mx-auto px-6 sm:px-10 lg:px-16">
          <div className="flex items-center justify-between h-20 lg:h-24">
            ...
            >
              ...
              >
                <path d={LOGO_FULL} fill="var(--fg)" />
              </motion.svg>
              <span className="hidden sm:block w-px h-5 bg-[var(--fg-extra-ghost)]" />
              <span className="hidden sm:block font-sans text-[0.7rem] tracking-[0.18em] uppercase text-[var(--fg)] group-hover:opacity-60 transition-opacity duration-300">
            ...
            <div className="hidden md:flex items-center gap-10">
              {navItems.map((item) => (
                <button
                  ...
                  className="relative font-sans text-[0.85rem] text-[var(--fg-secondary)] hover:text-[var(--fg)] transition-colors duration-300 group py-1"
                >
                  {item.label}
                  <span className="absolute bottom-0 left-0 w-full h-px bg-[var(--fg)] origin-left scale-x-0 group-hover:scale-x-100 transition-transform duration-500 ease-[cubic-bezier(0.16,1,0.3,1)]" />
                </button>
              ))}
              ...
              >
                <div className="w-6 flex flex-col gap-1.5">
                  <motion.span
                    ...
                    className="block h-[1.5px] w-full bg-[var(--fg)] origin-center"
                    transition={{ duration: 0.3 }}
                  />
                  <motion.span
                    animate={mobileMenuOpen ? { opacity: 0 } : { opacity: 1 }}
                    className="block h-[1.5px] w-full bg-[var(--fg)]"
                    transition={{ duration: 0.3 }}
                  />
                  <motion.span
                    animate={mobileMenuOpen ? { rotate: -45, y: -6 } : { rotate: 0, y: 0 }}
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            ...
            className="fixed inset-0 z-40 bg-[var(--bg)] flex flex-col justify-center items-center md:hidden"
          >
            <nav className="flex flex-col items-center gap-10">
              {navItems.map((item, index) => (
                <motion.button
                  ...
                  onClick={() => scrollTo(item.href)}
                  className="font-serif text-4xl text-[var(--fg)] hover:opacity-50 transition-opacity"
                >
                  {item.label}
                </motion.button>
              ))}
            ...
            <motion.a
              ...
              className="absolute bottom-12 font-sans text-sm text-[var(--fg-muted)] hover:text-[var(--fg)] transition-colors"
            >
